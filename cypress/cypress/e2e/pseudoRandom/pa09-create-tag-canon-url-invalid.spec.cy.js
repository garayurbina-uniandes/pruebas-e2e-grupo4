describe('Testing Tag Creation with Invalid data, Long String on description', () => {
    let imageSequence = 1;
    ...
    it('Create tag with invalid data', () => {
        ...
        cy.get('#tag-slug').type(randomPool['single_word'], { delay: 0}) 
        cy.get('span').contains('Expand').first().click();
        cy.wait(2000);
        cy.get('#canonical-url').type(randomPool['single_word'])// create Public tag with bad canon url
