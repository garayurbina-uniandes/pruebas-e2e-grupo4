describe('Como usuario creo una página con título problemático y validar que se agruegue al listado de páginas', () => {
    let imageSequence = 1;
    ...
    it('Create a page and verifies it exists', ()=> {
        ...
        cy.screenshot(`${prefixPath}/${imageSequence++}`, {overwrite: true});
        cy.wait(1000);
        cy.get('textarea.gh-editor-title.ember-text-area.gh-input.ember-view').type(randomPool['bad_string'])
        cy.screenshot(`${prefixPath}/${imageSequence++}`, {overwrite: true});
        cy.wait(1000);
